...
drawTownStreetDecor=function(){
  ...
  if(zone===0){
    ...
    X.globalAlpha=.3;
    X.fillStyle='#63b6cf';
    X.fillRect(1315,0,425,WH);
    X.globalAlpha=.6;
    X.fillStyle='#d9b77d';
    X.fillRect(1290,0,28,WH);X.fillRect(1740,0,28,WH);
    ...
    X.globalAlpha=.16;X.fillStyle='#eef7ff';
    for(let i=0;i<9;i++){let x=120+i*370,y=110+(i&1)*1320;X.beginPath();X.ellipse(x,y,190,75,0,0,T);X.fill()}
    X.globalAlpha=.65;X.strokeStyle='#d9edf5';X.lineWidth=6;
    X.beginPath();X.moveTo(430,260);X.lineTo(2740,1370);X.stroke();
    X.strokeStyle='#7b93a8';X.lineWidth=2;X.beginPath();X.moveTo(430,275);X.lineTo(2740,1385);X.stroke();
    for(let i=0;i<3;i++){let q=(clock*.035+i/3)%1,x=430+(2740-430)*q,y=260+(1370-260)*q;X.fillStyle=i&1?'#ffd36e':'#83d8ef';rr(x-22,y-13,44,26,8);X.strokeStyle='#334b5d';X.lineWidth=2;X.beginPath();X.moveTo(x,y-13);X.lineTo(x,y-48);X.stroke()}
  }
  X.restore();
};
...
drawTownBuilding=function(o,tall,d){
  ...
  if(zone===0){
    ...
    X.fillStyle='#2d7f93';X.globalAlpha=.72;X.fillRect(x+8,y+h-13,w-16,5);
    X.globalAlpha=.9;townCircle(x+22,y+34,8,'#b9edf7');X.strokeStyle='#356979';X.lineWidth=3;X.beginPath();X.arc(x+22,y+34,8,0,T);X.stroke();
  }else{
    X.globalAlpha=.8;X.fillStyle='#d7e8ef';X.fillRect(x+6,y+5,w-12,5);
    X.fillStyle='#8ec8dc';X.fillRect(x+w-17,y+28,6,Math.max(22,h-72));
    if((o.k||0)>4){X.strokeStyle='#dcecf3';X.lineWidth=3;X.beginPath();X.moveTo(x+w/2,y);X.lineTo(x+w/2,y-28);X.stroke();townCircle(x+w/2,y-31,4,'#ffd36e')}
  }
  X.restore();
};
...
drawPeople=function(){
  for(const p of people){
    const walk=Math.sin(clock*5+p.h)*4;
    townCircle(p.x,p.y,10,`hsl(${p.h} 42% 72%)`);
    X.fillStyle=zone===1?'#2f6f7d':zone===2?'#53697a':`hsl(${(p.h+140)%360} 45% 42%)`;
    rr(p.x-7,p.y+10,14,20,4);
    X.strokeStyle='#263242';X.lineWidth=4;X.lineCap='round';X.beginPath();
    X.moveTo(p.x-4,p.y+17);X.lineTo(p.x-12,p.y+25+walk*.2);
    X.moveTo(p.x+4,p.y+17);X.lineTo(p.x+12,p.y+25-walk*.2);
    X.moveTo(p.x-3,p.y+29);X.lineTo(p.x-8-walk,p.y+43);
    X.moveTo(p.x+3,p.y+29);X.lineTo(p.x+8+walk,p.y+43);X.stroke();
    if(zone===1){X.fillStyle='#f0c35b';X.fillRect(p.x-11,p.y-12,22,4)}
    if(zone===2){X.fillStyle='#d7e8ef';X.fillRect(p.x-8,p.y+11,16,4)}
  }
};
...
title=function(){
  showcaseTitleBase();
  if(state!=='title'||guide)return;
  X.fillStyle='#ffffffb8';
  text('POST-JS13K SHOWCASE EDITION',W-24,26,11,'right');
};
